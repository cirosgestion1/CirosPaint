...
class SearchThread(QThread):
    ...
    def __init__(self, api_key: str, original_query: str, search_query: str, parent=None):
    ...
    def run(self):
        try:
            results = YouTubeService(self.api_key).search_tutorials(self.original_query, self.search_query)
...
class TutorialSearchPage(QWidget):
    ...
    def search(self):
        ...
        self.status.setText(f"Buscando tutoriales para «{decision.original_query}»…")
        self.context_note.setText(f"Consulta contextualizada internamente: {decision.search_query}")
        thread = SearchThread(api_key, decision.original_query, decision.search_query, self)
        self._search_thread = thread
        thread.succeeded.connect(partial(self._show_results, decision.original_query))
    ...
    def _show_results(self, source_query: str, results):
        if not results:
            self.status.setText("No se han encontrado tutoriales suficientemente relacionados con el hobby.")
            return
        self.status.setText(f"{len(results)} resultados · relevancia → visitas → Me gusta → actualidad")
