...
class FavoritePaintAnalysisTests(unittest.TestCase):
    ...
    def test_possible_name_match_is_separate_from_exact(self):
        ...
        self.assertGreaterEqual(result.possible_matches[0].name_similarity, 0.78)

    def test_missing_paint_uses_same_type_and_delta_e_order(self):
        inventory = [
        ...
        self.assertEqual([item.inventory_paint.name for item in alternatives], ["Deep Red", "Scarlet"])
        self.assertTrue(all(item.inventory_paint.paint_type == "Acrílico" for item in alternatives))
        self.assertLess(alternatives[0].delta_e, alternatives[1].delta_e)
