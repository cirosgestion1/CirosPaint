...
class PaintAnalysisDialog(QDialog):
    def __init__(self, title: str, result: PaintAnalysisResult, parent=None):
    ...
    def _author_section(self) -> QFrame:
        ...
        if not self.result.author_lines:
            empty = QLabel("No se han encontrado pinturas identificables en la descripción del vídeo.")
    ...
    def _matches_section(self) -> QFrame:
        frame, layout = self._section(
            ...
            "Primero se muestran coincidencias exactas y después coincidencias probables por nombre.",
        )

        if not self.result.matches:
            empty = QLabel("No hay coincidencias exactas ni probables con pinturas disponibles en tu inventario.")
            empty.setObjectName("Muted")
            empty.setWordWrap(True)
            layout.addWidget(empty)
            return frame
        ...
        for match in self.result.exact_matches:
            item, item_layout = self._item_box()
            state = QLabel("✓ Coincidencia exacta")
    ...
    def _alternatives_section(self) -> QFrame:
        frame, layout = self._section(
            3,
            "Alternativas",
            "Solo aparecen cuando no hay coincidencia exacta ni probable. Antes del color se exige el mismo tipo de pintura.",
        )
        ...
        for missing in self.result.missing:
            ...
            if not missing.alternatives:
                none = QLabel("No hay una alternativa compatible con color comparable en tu inventario.")
