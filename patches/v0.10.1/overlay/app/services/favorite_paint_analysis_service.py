...
class FavoritePaintAnalysisService:
    ...
    0.10.1 intentionally does not use AI, captions, audio or video frames. A
    paint is only accepted when text in the public YouTube description can be
    tied to one unambiguous item in Ciros Paint's paint catalog.
    """
    ...
    def detect_catalog_paints(self, description: str) -> list[DetectedPaint]:
        ...
        for raw_line in (description or "").splitlines():
            ...
            # Long/strong aliases are considered first. Ambiguous aliases are
            # deliberately omitted from the index, so a generic name can never
            # silently select an arbitrary catalogue item.
    ...
    def _build_alias_index(cls, items: tuple[CatalogPaint, ...]) -> tuple[tuple[str, CatalogPaint, int], ...]:
        ...
        for alias, candidates in alias_map.items():
            ...
            if len(identities) != 1:
                # If the same bare alias points at several catalogue paints,
                # only a longer brand/range/code alias may identify it later.
                continue
    ...
    def _aliases_for_item(cls, item: CatalogPaint) -> tuple[tuple[str, int], ...]:
        ...
        if item.code:
            # Product codes are high-confidence when they include enough
            # information to avoid matching ordinary prose/numbers.
