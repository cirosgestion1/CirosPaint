...
class AssistantPaintService:
    ...
    def set_paint_quantity_by_id(self, paint_id: int, quantity: int) -> AssistantToolResult:
        """Set stock for an already resolved local entity without another text lookup."""
        paint = self.query_service.get_inventory_paint(int(paint_id))
        if paint is None:
            return AssistantToolResult("not_found", "La pintura activa ya no existe en el inventario.")
        try:
            quantity = int(quantity)
        except (TypeError, ValueError):
            return AssistantToolResult("invalid", "La cantidad debe ser un número entero.")
        if quantity < 0:
            return AssistantToolResult("invalid", "La cantidad total no puede ser negativa.")
        current_low = max(0, self._safe_int(getattr(paint, "low_units", 0)))
        paint.low_units = min(current_low, quantity)
        paint.available_units = quantity - paint.low_units
        self.session.commit()
        payload = self._paint_payload(paint)
        return AssistantToolResult(
            "ok", f"La cantidad total de {payload['brand']} {payload['name']} se ha establecido en {quantity}.",
            {"paint": payload, "total_units": quantity},
        )

    def add_paint_id_to_future_purchases(self, paint_id: int, quantity: int = 1) -> AssistantToolResult:
        """Reuse the existing ShoppingRepository for a resolved inventory paint ID."""
        paint = self.query_service.get_inventory_paint(int(paint_id))
        if paint is None:
            return AssistantToolResult("not_found", "La pintura activa ya no existe en el inventario.")
        quantity = max(1, int(quantity))
        self.shopping_repository.set_future_quantity(int(paint.id), quantity)
        payload = self._paint_payload(paint)
        return AssistantToolResult(
            "ok", f"{payload['brand']} {payload['name']} se ha añadido a Futuras compras.",
    ...
    def _resolve_catalog_paint(self, **filters: str):
        items = self.query_service.list_catalog_paints()
