...
def _language_badge(video: TutorialVideo, parent=None) -> QLabel:
    badge = QLabel(video.language_tag, parent)
    badge.setObjectName("TutorialLanguageTag")
    badge.setAlignment(Qt.AlignCenter)
    badge.setMinimumWidth(34)
    badge.setFixedHeight(26)
    badge.setToolTip(video.language_name)
    badge.setStyleSheet(
        "QLabel#TutorialLanguageTag {"
        " background: #182334;"
...
class TutorialCard(QFrame):
    ...
    def __init__(self, video: TutorialVideo, source_query: str, parent=None):
        ...
        self.language_badge = _language_badge(video, self)
        ...
        # Requested order: YouTube button -> language tag -> favorite star.
        buttons.addWidget(self.language_badge)
        buttons.addStretch()
...
class TutorialSearchPage(QWidget):
    def __init__(self):
        ...
        self.language_filter.currentIndexChanged.connect(self._language_changed)
        ...
        self.context_note = QLabel("Todos busca en español e inglés; los filtros de idioma se comprueban también sobre cada vídeo.")
    ...
    def _language_changed(self, _index: int):
        # Re-run an existing search immediately so changing the filter can never
        # leave stale Spanish results visible while "Inglés" is selected.
        if self._has_searched and self.input.text().strip() and not (self._search_thread and self._search_thread.isRunning()):
            self.search()

    def _context_text(self, language_code: str) -> str:
        if language_code == "es":
            return "Consulta contextualizada hacia el hobby y buscada en español; se descartan resultados que no se identifiquen como españoles."
        if language_code == "en":
            return "Consulta convertida a términos de hobby en inglés; se descartan resultados que no se identifiquen como ingleses."
        return "Búsqueda bilingüe: Ciros Paint consulta YouTube en español y en inglés y combina los candidatos por relevancia."
    ...
    def search(self):
        ...
        self.context_note.setText(self._context_text(language_code))
